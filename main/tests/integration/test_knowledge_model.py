...
def test_knowledge_base_timestamps(session: Session):
...
def test_document_chunk_relationship(session: Session):
...
def test_processing_task_defaults(session: Session):
...
def test_document_upload_cascade(session: Session):
    ...
    remaining = session.query(DocumentUpload).filter_by(id=upload.id).first()
